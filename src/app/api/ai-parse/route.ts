...
2. דיוק בפרטי הפונה (קריטי):
   - אם קיים כבר reporter_name או reporter_email או user_city בנתוני הטופס (currentFormData), שמור עליהם בדיוק כפי שהם ואל תשרשר או תשכפל אותם (למשל: אל תיצור "דניאל אורן | דניאל אורן").
   - אם המשתמש ציין עיר/מטה בטקסט (למשל "מטה חיפה"), עדכן את שדה user_city.
3. זהה האם חסרים פרטים מהותיים:
   - פירוט בסיסי המאפשר טיפול (מה התקלה או הבקשה הספציפית).
4. אם חסר פרט מהותי להבנת התקלה, החזר ב-follow_up_question שאלת המשך קצרה, אנושית, מנומסת וממוקדת בעברית.
5. ברגע שיש לך את המידע הנדרש לפתיחת הקריאה, החזר follow_up_question: null כדי שהמערכת תעביר את המשתמש מיד לטופס המלא לבדיקה ואישור.
6. קבע את הצוות המטפל (assigned_team) מתוך:
